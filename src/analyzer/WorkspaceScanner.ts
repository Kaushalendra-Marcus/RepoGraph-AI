...
export async function scanWorkspace(
  ...
  const allUris = await vscode.workspace.findFiles(
    pattern,
    "**/{node_modules,.git,dist,build,.next,__pycache__,.venv,venv,out,.output,.turbo}/**"
  );
  ...
  // Read in batches
  const batchSize = 20;
  for (let i = 0; i < Math.min(codeUris.length, 150); i += batchSize) {
    ...
    onProgress?.(`Read ${Math.min(done, codeUris.length)}/${Math.min(codeUris.length, 150)} files...`);
  }
